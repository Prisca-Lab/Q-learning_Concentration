...
class Game:
    ...
    def make_move(self, player):
        ...
        clicked_card, position, which_is_clicked = self.__select_card(available_cards, player)

        # if it's second turn check if a pair has been found
        match = self.__check_if_match(clicked_card, open_card_name)

        # update game data
        self.__update_game(clicked_card, position, match, which_is_clicked)

        return clicked_card, position, which_is_clicked, match
    ...
    def make_move_by_card(self, card, position):
        ...
        # in order to return which card's position has been clicked
        current_open = Card.get_which_is_open(card, game_board) 
        which_is_clicked = 'is_first_opened' if current_open == "is_second_opened" else 'is_second_opened'

        # in this case it is a 100% match because the robot suggested a card based on a face up card
        match = True
    ...
    def make_move_by_grid(self, type_of_suggest, number_suggested):
        """
        ...
        available_cards = Card.get_available_cards_by_suggest(type_of_suggest, number_suggested, game_board)
