...
class Agent:
    def __init__(self, env, type):
        ...
        if type == constants.HIDDEN_DECEPTION or type == constants.EXTERNAL_DECEPTION:
            self.sentence_generator_first_flip = SuggestionGenerator(tom_mode, language)
        else:
            self.sentence_generator_first_flip = None
    ...
    def __handle_first_flip(self):
        ...
        tom_modes = [constants.TOM, constants.EXTERNAL_DECEPTION, constants.HIDDEN_DECEPTION]
        ...
        elif self.type == constants.NO_TOM or self.type == constants.SUPERFICIAL_DECEPTION:
    ...
    def __handle_second_flip(self, action):
        ...
        if self.type == constants.SUPERFICIAL_DECEPTION:
    ...
    def generate_sentence(self, hint_type, flip_type, flag_ToM, card, position):
        ...
        """
        tom_modes = [constants.HIDDEN_DECEPTION, constants.EXTERNAL_DECEPTION]
        is_first_flip = flip_type == "firstCard"

        # if none then say nothing
        if hint_type == 'none': return None

        if self.type in tom_modes and is_first_flip:
            # Use ToM sentences if hint is provided for the first flip and mode is 'hidden' or 'external'
    ...
    def handle_deceptive_behavior(self, robot_type, card_name, position, action):
        ...
        deception_modes = [constants.DECEPTION, constants.EXTERNAL_DECEPTION]
        ...
        if robot_type in deception_modes and is_card_open:
            ...
            elif robot_type == constants.EXTERNAL_DECEPTION:
